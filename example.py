...
class color_picker:
    ...
    def color_chooser(self):
        ...
        self.R_scale = Scale(master = self.window, orient='horizontal', from_=0, to=255)
        self.R_scale.grid(row = 0, column = 0)
        self.G_scale = Scale(master = self.window, orient='horizontal', from_=0, to=255)
        self.G_scale.grid(row = 1, column = 0)
        self.B_scale = Scale(master = self.window, orient='horizontal', from_=0, to=255)
        ...
        self.Ok_button = Button(master = self.window, text = 'Okay', command = self.close_dialog).grid(row = 3, column = 0)
        self.close_button = Button(master = self.window, text = 'Close', command = self.window.destroy).grid(row = 3, column = 1)
